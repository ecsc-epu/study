...
class FlowchartRenderer {
  ...
  render(data, isSubRoadmap = false) {
    ...
    this.nodesContainer.classList.add("no-transitions");
    ...
    setTimeout(() => {
      this.nodesContainer.classList.remove("no-transitions");
    }, 50);
  }
  ...
  _createNodeElement(node, isSubRoadmap) {
    ...
    try {
      const emojiMatch = node.label.match(
      ...
      const simpleMatch = node.label.match(
        /^([\u2600-\u27BF\uD83C-\uDBFF\uDC00-\uDFFF]+)\s*/,
      );
      if (simpleMatch) {
    ...
    if (node.description) {
      el.appendChild(
        createElement("div", {
          className: "flow-node__desc",
          textContent: node.description,
        }),
      );
    }
    ...
    const hasChildren =
      ...
      this.data.edges.some((e) => e.from === node.id);
    if (hasChildren) {
    ...
    el.addEventListener("click", (e) => {
      ...
      if (hasChildren) {
        ...
        this.nodesContainer.classList.add("is-animating-layout");
  ...
  _createEdge(edge) {
    ...
    path.setAttribute(
      "marker-end",
      isLocked ? "url(#arrowhead-locked)" : "url(#arrowhead)",
    );
  ...
  _calculateAutoLayout() {
    ...
    const layoutTree = (nodeId, xLevel, isHidden, parentNode) => {
      ...
      const children = this.data.edges
      ...
      children.forEach((childId) => {
        ...
        layoutTree(childId, xLevel + 400, isHidden || isCollapsed, node);
      });
    };

    roots.forEach((root) => {
      layoutTree(root.id, startX, false, null);
      currentY += 200;
    });

    // Auto-center horizontally
    const visibleNodes = this.data.nodes.filter(
      (n) => !this.hiddenNodes.has(n.id),
    );
    if (visibleNodes.length > 0) {
      ...
      const titleY =
        this.data.titleY !== undefined ? this.data.titleY : minY - 200;
  ...
  _updatePositions() {
    ...
    Object.values(this.nodeElements).forEach(({ el, data: node }) => {
      const isHidden =
        this.data.layout === "tree" && this.hiddenNodes.has(node.id);
    ...
    if (titleContainer && this.data && this.data.nodes.length > 0) {
      ...
      const nodesForCenter =
        visibleNodes.length > 0 ? visibleNodes : this.data.nodes;
    ...
    paths.forEach((path) => {
      ...
      if (this.data.layout === "tree" && this.hiddenNodes) {
        const isHidden =
          this.hiddenNodes.has(fromId) || this.hiddenNodes.has(toId);
