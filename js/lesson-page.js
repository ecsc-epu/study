...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  let courseData;
  try {
    courseData = await loadDataJS(`data/${courseNode.courseFile}`, courseNode.courseVar);
  } catch (e) { return; }
  ...
  // Render breadcrumb
  renderBreadcrumb(courseId, courseData.title, lesson.content?.title || lesson.label);
...
function renderBreadcrumb(courseId, courseTitle, lessonTitle) {
  ...
  // Separator
  breadcrumb.appendChild(createElement('span', { className: 'breadcrumb__sep', textContent: '›' }));
...
function renderLesson(lesson, courseData) {
  ...
  const emojiMatch = lesson.label.match(/^(\p{Emoji_Presentation}|\p{Extended_Pictographic})\s*/u);
  if (emojiMatch) {
  ...
  // Body
  if (content.body && content.body !== 'Nội dung bài giảng sẽ được cập nhật sau.') {
  ...
  if (content.resources && content.resources.length > 0) {
    ...
    resources.appendChild(createElement('h3', {
    ...
    content.resources.forEach(res => {
      ...
      link.appendChild(createElement('span', { className: 'resource-link__icon', textContent: '📎' }));
...
function showError(message) {
  ...
  placeholder.appendChild(createElement('a', {
    className: 'lesson-back',
    href: 'index.html',
